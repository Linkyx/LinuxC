...
void display_attribute(struct stat buf, char *name)
{
    ...
    struct group *grp;      //从该结结构体中获取文件所有者所在组名
...
void display(int flag, char *pathname)
{
    ...
    switch(flag)
    {
        case PARAM_NONE:        //ls无参数
            if(name[0] != '.')
            {
                display_single(name);
            }
            break;
        case PARAM_A:           //ls参数为-a
            display_single(name);
            break;
        case PARAM_L:           //ls参数为-l
            if(name[0] != '.')
            {
            ...
            break;
        case PARAM_A + PARAM_L: //ls参数为-a，-l两个
            display_attribute(buf, name);
            printf("  %-s\n", name);
            break;
    }
...
void display_dir(int flag_param, char *path)
{
    ...
    //使用冒泡法对文件名进行排序，排序后文件名按字母顺序存于filename中
    for(i = 0; i<count-1; i++)
        for(j = 0; j<count-1-i; j++)
        {
            if(strcmp(filename[j], filename[j+1]) > 0)
            {
                strcmp(temp, filename[j+1]);
...
int main(int argc, char *argv[])
{
    ...
    int flag_param = PARAM_NONE;//判断是否有-l -a选项
    struct stat buf;
    /*命令行参数解析,分析-l -a -al -la选项*/
    j = 0;
    num = 0;
    for(i = 1; i<argc; i++)
    {
        ...
        {
            {
                for(k = 1; k<strlen(argv[i]); k++,j++)
                    param[j] = argv[i][k];//获取-后面的参数保存到param数组中
            }
            num++;//保存-的个数
        }
    }
